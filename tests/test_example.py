...
def test_nem12_data_monthly():
    ...
    month_summaries = get_monthly_charges(INTERVAL_READINGS, 'ergon', 't14')
    dec = month_summaries[(2016, 12)]
    assert dec.days == pytest.approx(31)
    assert dec.total == pytest.approx(176.64)


def test_nem13_data():
    """ Test example NEM13 summary """
    month_summaries = get_monthly_charges(MANUAL_READINGS, 'ergon', 't11')
    july = month_summaries[(2016, 7)]
    assert july.days == pytest.approx(31)
    assert july.total == pytest.approx(208, rel=1e-1)
